...
// TODO: Support for VIDIOC_DBG_G_REGISTER, VIDIOC_DBG_S_REGISTER
// TODO: Support for  VIDIOC_S_CTRL and VIDIOC_G_CTRL, VIDIOC_G_EXT_CTRLS, VIDIOC_S_EXT_CTRLS, VIDIOC_TRY_EXT_CTRLS
			...
			struct v4l2_format *vfmt)
{
	struct atmel_isi_fh *fh = priv;
	struct atmel_isi *isi = fh->isi;
   struct atmel_isi_format tmp_fmt = isi->format;
...
static int atmel_isi_init(struct atmel_isi *isi){
    ...
    * first */
	...
	if (!isi->camera) {
		...
		if (isi->camera && isi->camera->reprogram)
			isi->camera->reprogram(isi->camera);
...
static int __init atmel_isi_probe(struct platform_device *pdev)
{
   ...
   if (pdata && pdata->bootmemSize >= capture_buffer_size
      ...
      isi->bootmemBuff = 1;
	   ...
	   if (!isi->capture_buf) {
         ...
         isi->capture_buf = kmalloc(capture_buffer_size, GFP_KERNEL);
         if (!isi->capture_buf) {
            printk("kmalloc failed!\n");
         }
         else {
            printk("kmalloc returned address %08X\n", isi->capture_buf);
            kfree(isi->capture_buf);
            isi->capture_buf = NULL;
         }
	...
	*/		
	...
	if (ret){
		...
		kfree(dev);		
		pr_debug(KERN_INFO "failed to register streaming\n");		
		goto err_register2;
	}
